...
class P0Autopilot:
    ...
    def _write_audio(self) -> None:
        if self.audio_path.is_file() and self.audio_path.stat().st_size > 0:
            return
        self.audio_path.parent.mkdir(parents=True, exist_ok=True)
        temporary = self.audio_path.with_suffix(".wav.tmp")
        sample_rate = 44_100
        amplitude = 8_000
        frame_count = int(PROOF_DURATION_SECONDS * sample_rate)
        with wave.open(str(temporary), "wb") as output:
            output.setnchannels(2)
            output.setsampwidth(2)
            output.setframerate(sample_rate)
            for index in range(frame_count):
                sample = int(amplitude * math.sin(2.0 * math.pi * 110.0 * index / sample_rate))
...
def self_test() -> int:
    ...
    print("CinemaP0Autopilot PASS idempotent-proof=validated audio=validated secrets=absent resume=required")
    return 0
