...
if (-not $workspaceDrive -or $workspaceDrive.TrimEnd("\").ToUpperInvariant() -eq "C:") {
    throw "Echoes Cinema supervisor refuses workspace storage on drive C:. Current: $workspace"
}
...
try {
    ...
    throw "Another Echoes Cinema supervisor already owns $lockPath. Use START_ECHOES_CINEMA.cmd to reopen its dashboard."
}
...
try {
    ...
    if ($servicePort -eq $providerPort) {
        $providerPort = Find-FreePort -Preferred ($providerPort + 1)
    }
    ...
    $serviceToken = New-RandomToken
    $providerToken = New-RandomToken
    ...
    while (-not (Test-Path -LiteralPath $stopSignalPath)) {
        ...
        if (-not $serviceProcess -or $serviceProcess.HasExited) {
            $lastError = "Control center process stopped and is being restarted automatically."
    ...
    try {
        if (Get-Variable -Name dashboardUrl -ErrorAction SilentlyContinue) {
            Write-AtomicJson -Path $statePath -Payload @{
                schema = "echoes.cinema-stack-state.v1"
                status = "BROKEN"
                timestampUtc = [DateTime]::UtcNow.ToString("o")
    ...
    Stop-OwnedProcess -Process $providerWorkerProcess
    if (Test-Path -LiteralPath $providerPidPath) {
    ...
    Stop-OwnedProcess -Process $serviceProcess
