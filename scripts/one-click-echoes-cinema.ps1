...
function Write-Step {
    ...
    Write-Host $line
    if (Test-Path -LiteralPath $logs -PathType Container) { Add-Content -LiteralPath $logPath -Value $line -Encoding utf8 }
}

function Invoke-Git {
    param([string[]]$Arguments)
    & git @Arguments
    if ($LASTEXITCODE -ne 0) { throw ("Git failed with exit code {0}: git {1}" -f $LASTEXITCODE, ($Arguments -join ' ')) }
}
...
foreach ($directory in @($workspace, $logs, $runtime, $backups, $tempRoot, $proofRoot)) { New-Item -ItemType Directory -Path $directory -Force | Out-Null }
...
try {
    ...
    if ([System.IO.Path]::GetPathRoot($workspace).TrimEnd("\").ToUpperInvariant() -eq "C:") { throw "Echoes Cinema workspace refuses heavy storage on drive C:." }
    if (-not (Test-Path -LiteralPath (Join-Path $repo ".git") -PathType Container)) { throw "EchoesEngine Git repository is missing: $repo" }
    ...
    $pythonCandidates = @((Join-Path $workspace ".venv-cinema\Scripts\python.exe"), "D:\A.I\Python310\python.exe")
    ...
    if ($LASTEXITCODE -ne 0) { throw "Echoes Cinema startup failed with exit code $LASTEXITCODE" }
    ...
    if (-not (Test-HttpReady -Url $dashboardUrl)) { throw "The control center did not become reachable. Exact log: $logPath" }
    ...
    $monitorArguments = @("-NoProfile", "-ExecutionPolicy", "Bypass", "-File", $monitorScript, "-WorkspaceRoot", $workspace, "-MaximumMinutes", "240")
