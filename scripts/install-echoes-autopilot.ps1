...
function Resolve-CompatiblePython([string]$RequestedExecutable) {
    $candidates = @()
    if ($RequestedExecutable) { $candidates += [pscustomobject]@{ Executable = $RequestedExecutable; Prefix = @() } }
    ...
    foreach ($candidate in $candidates) {
        try {
            $details = & $candidate.Executable @($candidate.Prefix) -c "import json,sys; print(json.dumps({'version':f'{sys.version_info.major}.{sys.version_info.minor}','executable':sys.executable}))" 2>$null
            if ($LASTEXITCODE -ne 0 -or -not $details) { continue }
            $resolved = $details | ConvertFrom-Json
            if ($resolved.version -in @("3.10", "3.11") -and (Test-Path -LiteralPath $resolved.executable -PathType Leaf)) {
                return [pscustomobject]@{ Executable = [IO.Path]::GetFullPath([string]$resolved.executable); Version = [string]$resolved.version }
            }
        } catch { continue }
    }
    return $null
...
function Ensure-WingetPackage([string]$PackageId, [string]$Label) {
    ...
    $common = @("install", "--id", $PackageId, "--exact", "--silent", "--accept-package-agreements", "--accept-source-agreements", "--disable-interactivity")
    & $winget.Source @($common + @("--scope", "user"))
...
function Copy-AudioIntoInbox([string]$Path, [string]$Inbox) {
    ...
    if ($SupportedExtensions -notcontains $extension) { return $null }
    $sha = Get-Sha256 $full
    ...
    if (-not (Test-Path -LiteralPath $destination -PathType Leaf)) { Copy-Item -LiteralPath $full -Destination $destination }
    return $destination
}
...
$installedCore = Join-Path $autopilot "echoes_autopilot.py"
...
Copy-Item -LiteralPath (Join-Path $source "tools\echoes_autopilot.py") -Destination $installedCore -Force
...
$seedCandidates = @($InputPaths)
$seedFolder = Join-Path $source "SONGS-TO-ANALYZE"
if (Test-Path -LiteralPath $seedFolder -PathType Container) {
    $seedCandidates += @(Get-ChildItem -LiteralPath $seedFolder -File -ErrorAction SilentlyContinue | Select-Object -ExpandProperty FullName)
}
$copiedInputs = @()
foreach ($candidate in $seedCandidates) {
    $copied = Copy-AudioIntoInbox $candidate $inbox
    if ($copied) { $copiedInputs += $copied }
}
$copiedInputs = @($copiedInputs | Sort-Object -Unique)
...
$manifest = [ordered]@{
    ...
    seededInputs = $copiedInputs
    fileSha256 = [ordered]@{
        controller = Get-Sha256 $installedController
        core = Get-Sha256 $installedCore
        policy = Get-Sha256 $installedPolicy
        runtimeManifest = Get-Sha256 $runtimeManifestPath
    }
...
if (-not $NoInitialRun) {
    ...
    if (-not $NoOpen) { $controllerArguments += "-Interactive" }
